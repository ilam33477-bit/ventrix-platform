...
function ProblemDetailPanel({ api, problem, onChanged, onClose }: {
  ...
  return <Card className="problem-detail">
    ...
    <p><strong>Рабочий аккаунт:</strong> {problem.connection_name ?? "не определён"}{problem.connection_username ? ` · @${problem.connection_username}` : ""}</p>
    <p><strong>Переписка:</strong> {problem.dialog_title ?? "диалог"}{problem.dialog_username ? ` · @${problem.dialog_username}` : ""}</p>
...
export function ProblemsView({ api }: { api: VentrixClientApi }) {
  ...
  return <>
    ...
    {loading ? <Skeleton lines={4} /> : <div className="problem-cards">{visible.map((problem) => <button className="problem-card-button" key={problem.id} onClick={() => void open(problem.id)}><Card className="problem-card"><div className="problem-card-top"><StatusBadge tone={problem.priority === "critical" ? "danger" : "warning"}>{problem.priority === "critical" ? "Критично" : problem.priority === "high" ? "Важно" : "Проверить"}</StatusBadge><small>{STATUS_LABELS[problem.status] ?? problem.status}</small></div><h3>{problemTitle(problem)}</h3><p>{problem.explanation.replaceAll("SLA", "установленного времени ответа")}</p><p className="problem-origin"><strong>{problem.responsible_employee_name ?? "Ответственный не назначен"}</strong>{problem.connection_name ? ` · ${problem.connection_name}` : ""}{problem.dialog_username ? ` · @${problem.dialog_username}` : ""}</p><blockquote>{problem.evidence}</blockquote><p><strong>Что сделать:</strong> {problem.recommended_action}</p><footer>{problem.deadline_at ? `До ${new Date(problem.deadline_at).toLocaleString("ru-RU")}` : "Без жёсткого срока"}</footer></Card></button>)}</div>}
    {error && <p className="form-error">{error}</p>}
