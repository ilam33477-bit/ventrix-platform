...
export function ConnectionManager({ api, connections: initialConnections, onboardingStep, onOnboardingStep, mode = "manage", onSkip, assignedEmployee, createEmployee = false, onComplete }: {
  ...
  const [, setCodePhoneMasked] = useState(
  ...
  return <section className={`connection-feature ${mode}`}>
    ...
    <div className="connection-layout">
      {showConnection && <Card className="connection-wizard">
        ...
        {step === "confirm_phone" && <><h3>Проверьте номер</h3><p>Код входа будет запрошен для номера:</p><div className="phone-confirmation">{phone}</div><button className="primary-action" disabled={busy} onClick={startLogin}>{busy ? "Запрашиваем код…" : "Отправить код"}</button><button className="text-action" disabled={busy} onClick={() => setStep("phone")}>Изменить номер</button></>}
        {step === "code" && <><h3>Код подтверждения</h3><p>Введите полученный код. Он используется один раз и не сохраняется.</p><label>Код<input className="code-input" inputMode="numeric" autoComplete="one-time-code" enterKeyHint="done" maxLength={8} value={code} onChange={(event) => { setCode(event.target.value.replace(/\D/g, "")); setError(null); }} /></label><button className="primary-action" disabled={busy || code.length < 4} onClick={() => completeLogin(false)}>{busy ? "Проверяем…" : "Продолжить"}</button><div className="secondary-actions"><button disabled={busy || !code} onClick={() => { setCode(""); setError(null); }}>Очистить</button><button disabled={busy || resendAvailableIn > 0} onClick={resendCode}>{resendAvailableIn > 0 ? `Новый код через ${resendAvailableIn} сек.` : "Отправить новый код"}</button></div></>}
